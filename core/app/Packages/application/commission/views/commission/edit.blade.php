@extends('layouts.master') @section('title','Edit Commission')
@section('css')
	<link rel="stylesheet"
...
@section('content')
	<ol class="breadcrumb">
		...
		<li>
			<a href="javascript:;">Commission Management</a>
		</li>
		<li>
			<a href="{{url('repTarget/list')}}" >List Commission</a>
		</li>
				...
				<div class="panel-body">
					<form role="form" class="form-horizontal form-validation" method="post">
						...
						<div class="form-group">
							...
							<div class="col-sm-6">
								 <input type="text" disabled class="form-control" name="rep" required value="{{$commission->employee->first_name .' '.$commission->employee->last_name}}">
							        <input type="hidden" name="empId" id="empId" value="{{$commission->employee->id}}">
							</div>
						</div>
						<div class="form-group">
                            ...
                            <div class="input-group">
                                <?php $arr = explode('-', $commission->from);
                                $newDate = $arr[0].'-'.$arr[1];?>
					...
					<div class="form-group">
							...
							<div class="col-sm-6">
								<input type="text" class="form-control @if($errors->has('value')) error @endif" name="value" placeholder="Input value" required value="{{$commission->value}}" onkeydown="return validValue(this, event)">
								@if($errors->has('value'))
...
//			});
		...
		/*	var date = new Date();
			if(Date.parse(date) > Date.parse('{{$commission->to}}')) {
