...
@section('css')
<link rel="stylesheet" type="text/css" href="{{asset('assets/vendor/bootstrap-switch-master/dist/css/bootstrap3/bootstrap-switch.min.css')}}">
<link rel="stylesheet" type="text/css" href="{{asset('assets/vendor/jquery-toast-plugin-master/dist/jquery.toast.min.css')}}">
                        ...
                        <div class="row">
                            <div class="col-sm-2">
                                <div class="input-group">
                                    <input type="text" autocomplete="off" class="form-control" name="invoice_no" id="invoice_no" autofocus placeholder="search.." value="{{$invoice_no}}" />
                        ...
                        <div class="row">
                            <div class="col-md-3">
                                <select class="chosen" name="customer">
                                    ...
                                    <option value="{{$customerObj->id}}" @if($customerObj->id == $customer ) selected @endif>{{$customerObj->f_name .' '.$customerObj->l_name}}
                                    </option>
                                    @endforeach
                            ...
                            <div class="col-md-3">
                                <select class="chosen" name="marketeer">
                                    ...
                                    <option value="{{$marketeerObj->id}}" @if($marketeerObj->id == $marketeer ) selected @endif>{{$marketeerObj->first_name .' '.$marketeerObj->last_name}}
                                    </option>
                                    @endforeach
                            ...
                            <div class="col-md-2 input-daterange">
                                <input type="text" autocomplete="off" class="form-control" name="from" id="from" placeholder="From Date" value="{{$from}}" />
                            </div>
                            <div class="col-md-2 input-daterange">
                                <input type="text" autocomplete="off" class="form-control" name="to" id="to" placeholder="To Date" value="{{$to}}" />
                            </div>
                            <div class="col-md-2">
                                <div class="pull-left">
                                    <button type="submit" class="btn btn-info" id="plan"><i class="fa fa-search" style="padding-right: 16px;width: 28px;"></i>Find
                                    </button>
                                </div>
                                <div class="pull-right" style="padding-right: 10px">
                                    <a href="list" class="btn btn-warning" data-toggle="tooltip" data-placement="top"><i class="fa fa-refresh"></i></a>
    ...
    <div class="col-md-12">
        <div class="box box-widget box-list box-primary">
            <div class="box-body" style="max-height: 500px;min-height: 500px;overflow-y: scroll;">
                ...
                <button type="submit" class="btn btn-info">Sales <i class="fa fa-file-pdf-o"></i>
                </button>
                </form>
            </div>
        </div>
        ...
        <div class="col-md-1 pull-right">
            <div class="pull-right" style="padding-right: 10px;padding-top: 10px;padding-left: 10px">
                <form role="form" method="get" action="{{url('invoice/payment-aging')}}">
                    ...
                    <input type="hidden" name="from" value="{{$from}}">
                    <input type="hidden" name="to" value="{{$to}}">
                    <button type="submit" class="btn btn-warning">Invoice Aging <i class="fa fa-file-pdf-o"></i>
                    </button>
                </form>
        ...
        <div class="col-md-1 pull-right">
            ...
            <div class="pull-right" style="padding-right: 10px;padding-top: 10px;padding-left: 10px">
                <form role="form" method="get" action="{{url('job/download')}}">
                    ...
                    <input type="hidden" name="from" value="{{$from}}">
                    <input type="hidden" name="to" value="{{$to}}">
                    <button type="submit" class="btn btn-info">Jobs <i class="fa fa-file-pdf-o"></i>
                    </button>
                </form>
            </div>
        </div>
        ...
        <table class="table table-bordered bordered table-striped table-condensed" id="orderTable">
            ...
            <tbody>
                ...
                <tr id="{{$i}}">
                    <td>{{$i}}</td>
                    <td>
                        <a href="{{url('job/print/'.$order->id)}}" class="inv_link" target="_blank" data-toggle="tooltip" data-placement="top" title="Job Processing"><span><i class="fa fa-link"></i> {{$order->job_no}}</span></a>
                        <br>
                    ...
                    <td>
                        Created:</br>{{$order->created_at}} </br>
                        Last Updated: </br>{{$order->updated_at}}
                    </td>
                    <td>{{$order->due_date}}</td>
                    <td>{{$order->section}}</td>
                    <td>@if(strlen($order->ended_at) == 0)
                        <span class="badge" style="border-radius: 0px">{{$order->confirm_type ?: 'JOB CREATOR'}}
                            CONFIRMED</span>
                        @else
                    ...
                    <td style="text-align: center">
                        @if($order->section == $marketeerList[0]->type->type || $user->hasAnyAccess(['job.add']))
                        @if($user->hasAnyAccess(['job.change']))
                        <a data-toggle="tooltip" data-placement="top" href="{{url('job/edit/'.$order->id)}}" title="Edit Job" style="padding-left: 2px;cursor: hand">
                        ...
                        <a data-toggle="tooltip" data-placement="top" href="{{url('job/confirm/'.$order->id)}}" title="Confirmation" style="padding-left: 2px;cursor: hand">
                        ...
                        <a @if($user->id == 1 || trim($user->roles[0]->name) != 'customer') href="{{url('job/qr/list/'.$order->id)}}"
                            @endif target="_blank" title="print QR" style="padding-left: 2px;cursor: hand"">
                            <span class="badge btn-info" style="border-radius: 0px">
                                <i class="fa fa-qrcode fa-lg"></i>
                            </span>
                        </a>
                        <a @if($user->id == 1 || trim($user->roles[0]->name) != 'customer')
                            @endif title="See Attachments" style="padding-left: 2px;cursor: hand"">
                            <span class="badge btn-primary" style="border-radius: 0px" onclick="seeModel({{($order->id)}})">
                        ...
                        <div class="modal-dialog" role="document">
                            <div class="modal-content">
                                <div class="modal-header">
                                    ...
                                    <h4 class="modal-title" id="exampleModalLabel">Attachments</h4>
                                </div>
                                <div class="modal-body">
                                    <form>
                                        <div class="form-group" style="font-size: medium;">
                                            <label for="recipient-name" class="control-label">01. Attachment = </label>
                                            @if(($order->attachment_1) != null)
                                            <a href="{{url($order->attachment_1)}}" target="_blank"> <i class="bi bi-eye"></i> </a>
                                            @else
                                            <a href="#" style="cursor: not-allowed;"> <i class="bi bi-eye-slash"></i> </a>
                                            @endif
                                        </div>
                                        <div class="form-group" style="font-size: medium;">
                                            <label for="recipient-name" class="control-label">02. Attachment = </label>
                                            @if(($order->attachment_2) != null)
                                            <a href="{{url($order->attachment_2)}}" target="_blank"> <i class="bi bi-eye"></i> </a>
                                            @else
                                            <a href="#" style="cursor: not-allowed;"> <i class="bi bi-eye-slash"></i> </a>
                                            @endif
                                        </div>
                                        <div class="form-group" style="font-size: medium;">
                                            <label for="recipient-name" class="control-label">03. Attachment = </label>
                                            @if(($order->attachment_3) != null)
                                            <a href="{{url($order->attachment_3)}}" target="_blank"> <i class="bi bi-eye"></i> </a>
                                            @else
                                            <a href="#" style="cursor: not-allowed;"> <i class="bi bi-eye-slash"></i> </a>
                                            @endif
                                        </div>
                                        <div class="form-group" style="font-size: medium;">
                                            <label for="recipient-name" class="control-label">04. Attachment = </label>
                                            @if(($order->attachment_4) != null)
                                            <a href="{{url($order->attachment_4)}}" target="_blank"> <i class="bi bi-eye"></i> </a>
                                            @else
                                            <a href="#" style="cursor: not-allowed;"> <i class="bi bi-eye-slash"></i> </a>
                                            @endif
                                        </div>
                                        <div class="form-group" style="font-size: medium;">
                                            <label for="recipient-name" class="control-label">05. Attachment = </label>
                                            @if(($order->attachment_5) != null)
                                            <a href="{{url($order->attachment_5)}}" target="_blank"> <i class="bi bi-eye"></i> </a>
                                            @else
                                            <a href="#" style="cursor: not-allowed;"> <i class="bi bi-eye-slash"></i> </a>
                                            @endif
                                        </div>
...
<script>
    // Define the seeModel function
    function seeModel(id) {
        $('#' + id).modal('show')
    }
</script>
...
<script>
    var tpl_order = new EJS({
        url: '{{url('
        core / app / Packages / application / purchaseOrder - management / views / template / tpl_order ')}}'
    });
    var tpl_invoice_no = new EJS({
        url: '{{url('
        core / app / Packages / application / purchaseOrder - management / views / template / tpl_invoice_no ')}}'
    });
    var tpl_invoice_row = new EJS({
        url: '{{url('
        core / app / Packages / application / purchaseOrder - management / views / template / tpl_invoice_row ')}}'
    });
    var tpl_reject = new EJS({
        url: '{{url('
        core / app / Packages / application / purchaseOrder - management / views / template / tpl_reject ')}}'
    });
