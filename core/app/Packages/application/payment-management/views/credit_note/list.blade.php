@extends('layouts.master') @section('title','Credit Note List')
@section('css')
    ...
    <div class="col">
        <div class="col-xs-12">
            <div class="box box-widget box-list box-primary">
                <div class="box-body">
                    <form role="form" method="get" action="{{url('payment/report/credit-note/list')}}">
                                ...
                                <div class="col-sm-3">
                                    <div class="input-group">
                                        <input type="text" autocomplete="off" class="form-control"
                                               name="no"
                                               id="no"
                                               placeholder="Invoice No" value="{{$no}}"/>
            ...
            <div class="box box-widget box-list box-primary">
                <div class="box-body" style="max-height: 500px;min-height: 500px;overflow-y: scroll;">
                    ...
                    <div class="form-group">
                            <form role="form" method="get" action="{{url('payment/report/credit-note/download')}}">
                            ...
                            <form role="form" method="get" action="{{url('payment/report/credit-note/excel')}}">
                                <div class="col-md-1 pull-right">
                                    <div class="pull-right"
                                         style="padding-right: 10px;padding-top: 10px;padding-left: 10px">
                                        <input type="hidden" name="no" value="{{$no}}">
                    ...
                    <div class="form-group" style="margin-top: 50px" >
                        <table class="table table-bordered bordered table-striped table-condensed"
                            ...
                            @foreach($orders as $order)
                                <tr id="{{$i}}">
                                    <td>{{$i}}</td>
                                    <td>{{$order->invoice->customer->f_name.' '.$order->invoice->customer->l_name}}</td>
                                    <td>
                                        <span><i class="fa fa-thumb-tack"></i> {{$order->invoice_no}}</span><br>
                                    </td>
                                    <td>{{$order->created_at}}</td>
                                    <td style="text-align: right">{{number_format($order->credit_amount,2)}}</td>
                                    <td class="rmk">{{$order->comment}}</td>
                                    <td class="rmk">{{$order->create_user->first_name}} {{$order->create_user->last_name}}</td>
