@extends('layouts.master') @section('title','Online Payment List')
@section('css')
    ...
    <div class="col">
        <div class="col-xs-12">
            <div class="box box-widget box-list box-primary">
                <div class="box-body">
                    <form role="form" method="get" action="{{url('payment/report/onlinePayment/list')}}">
                        <div class="form-group" style="padding-left: 10px;padding-top: 10px;padding-bottom: 10px">
                            <div class="row">
                                <div class="col-sm-3">
                                    <div class="input-group">
                                        <input type="text" autocomplete="off" class="form-control"
                                               name="no"
                                               id="no"
                                               placeholder="Receipt No" value="{{$no}}"/>
                                ...
                                <div class="col-md-5">
                                    <select class="chosen" name="account">
                                        <option value="0">-- Search Account --
                                        </option>
                                        <?php $i = 0;?>
                                        @foreach($AccountList as $account_)
                                            <option value="{{$account_->id}}"
                                                    @if($account_->id == $account) selected @endif>{{$account_->account_no .' ( '.$account_->banks->name.' - '.$account_->branch.' )'}}
                                            </option>
                                        @endforeach
                                    </select>
                                </div>
            ...
            <div class="box box-widget box-list box-primary">
                <div class="box-body" style="max-height: 500px;min-height: 500px;overflow-y: scroll;">
                    ...
                    <div class="form-group">
                            <form role="form" method="get" action="{{url('payment/report/onlinePayment/download')}}">
                            ...
                            <form role="form" method="get" action="{{url('payment/report/onlinePayment/excel')}}">
                                <div class="col-md-1 pull-right">
                                    <div class="pull-right"
                                         style="padding-right: 10px;padding-top: 10px;padding-left: 10px">
                                        <input type="hidden" name="no" value="{{$no}}">
                        ...
                        <table class="table table-bordered bordered table-striped table-condensed"
                            ...
                            @foreach($orders as $order)
                                <tr id="{{$i}}">
                                    ...
                                    <td>
                                        {{$order->account ? $order->account->account_no : ''}}<br>
                                        {{$order->account ? $order->account->banks->name.' - '.$order->account->branch : ''}}
                                    </td>
