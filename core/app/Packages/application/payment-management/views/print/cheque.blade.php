            ...
            <table class="details" width="100%" style="border-bottom-style:solid">
                <thead style="background-color:rgba(52, 73, 94,0.5);color:#fff;">
                    <tr>
                        ...
                        <th width="15%" class="border-top">Customer</th>
                        <th align="left" width="30%" class="border-top">Receipt</th>
                        <th align="left" width="12%" class="border-top">Date</th>
                        <th align="right" width="10%" class="border-top">Amount</th>
                        <th align="left" width="10%" class="border-top">Status</th>
                ...
                @foreach($orders as $order)
                    <tr>
                        ...
                        <td class="border-top">Receipt -> {{$order->recipt_no}}<br>Cheque -> {{$order->cheque_no}}<br>Bank -> {{$order->name}}
                        </td>
                        <td class="border-top">{{$order->recipt_date}}</td>
                        <td class="border-top" style="text-align: right">{{number_format($order->cheque_amount,2)}}</td>
                        <td class="border-top">
                            @if($order->status == 1)
                                <b>Pending</b>
                    ...
                    <?php $i++;$total+=$order->cheque_amount;?>
                @endforeach
                </tbody>
                <tfoot>
                <tfoot>
                <tr>
                    <td class="border-top" colspan="3" style="font-size: 9px" align="right"><b>Total:</b></td>
                    <td class="border-top" colspan="2" align="right" style="font-size: 9px"><b>{{number_format($total,2)}}</b></td>
