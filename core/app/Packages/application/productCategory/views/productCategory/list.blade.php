@extends('layouts.master') @section('title','Category List')
@section('css')
<style type="text/css">
  .switch.switch-sm{
    width: 30px;
  ...
  <div class="col-xs-12">
    <div class="panel panel-bordered">
           <div class="panel-heading border">
            <div class="row"><div class="col-xs-6"><strong>Category List</strong></div>
            <div class="col-xs-6 text-right">
                @if($permissions)
            <button class="btn btn-success" onclick="window.location.href='{{url('productCategory/add')}}'"><i class="fa fa-plus" style="width: 28px;"></i>Add</button>
                    @endif
            </div>
            </div>
          </div>
            <div class="panel-body">
              <table id="print_table" name="print_table" class="table table-bordered bordered table-striped table-condensed datatable">
...
<script type="text/javascript">
  ...
  $(document).ready(function(){
    table = generateTable('.datatable', '{{url('productCategory/json/list')}}',[],[]);
    ...
    table.on('draw.dt',function(){
      ...
      $('.datatable .menu-activate').change(function(){
        if($(this).prop('checked')==true){       
          ajaxRequest( '{{url('productCategory/status')}}' , { 'id' : $(this).val() , 'status' : 1 }, 'post', successFunc);
        }else{
          ajaxRequest( '{{url('productCategory/status')}}' , { 'id' : $(this).val() , 'status' : 0 }, 'post', successFunc);
        }
      });
    });
  });
