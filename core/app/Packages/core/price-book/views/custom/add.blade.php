		            			...
		            			<table class="table table-bordered bordered table-striped table-condensed">
		            				<thead>
				                        <tr>
				                            ...
				                            <th rowspan="2" class="text-center" style="font-weight:normal;">Custom Price</th></th>
				                        </tr>
	                        		</thead>
...
<script type="text/javascript">
	...
	function load_user(){
		...
		$.ajax({
			url: "{{url('price/custom/json/userList')}}",
			type: 'GET',
			data: {'book_category': $("#book_category").val()},
	...
	function save_data(){
		...
		if($("#excel_radio").is(':checked')){
			...
			if($("#input-ficons-1").val()!=""){
				...
				detail=sCSV;				
				$.ajax({
					url: "{{url('price/custom/check-mrp')}}",
					type: 'GET',
					data: {'detail': detail},
					async: false,
					success: function(data) {		
		...
		if(detail.length>0 && $("#name").val()!="" && $("#effective_date").val()!="" && l==0 && m==0 && $("#book_category").val()!=0){
			...
			if($("#user").val()==null){
				...
				swal({
				    title: "Are you sure?",
				    text: "No users selected for price book! \n Do you want to continue?",
				    type: "warning",
				...
				function(isConfirm){
					if (isConfirm){
						if(n==0){
							send_data(basic,detail,[],clue);				  		
						}else{
							$('.panel').removeClass('panel-refreshing');
							swal("Warning", "Missing MRP of some product", "error");
						}
				    } else {
				    	$('.panel').removeClass('panel-refreshing');
				      	swal("Warning", "Please select user for price book", "error");
				    }
				});
			}else{
				if(n==0){
					send_data(basic,detail,$("#user").val(),clue);
				}else{
					$('.panel').removeClass('panel-refreshing');
					str=" ";
	...
	function send_data(basic,detail,users,clue){
		$.ajax({
			url: "{{url('price/custom/add')}}",
			type: 'POST',
			data: {'basic':basic,'detail': detail,'users': users,'clue': clue},
			async: false,
			success: function(data) {				
				if(data==1){
					$('.panel').removeClass('panel-refreshing');
					swal({ 
					    ...
					    window.location.href = "{{url('price/custom/add')}}";
					});					
				}else{
					$('.panel').removeClass('panel-refreshing');
